...
function destroy()
  if projectile.sourceEntity() and world.entityExists(projectile.sourceEntity()) then
    ...
    world.spawnProjectile("electricdoomshot", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 50, power = projectile.getParameter("power")})
    world.spawnProjectile("tezansbidenblast", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 20, power = projectile.getParameter("power")})
