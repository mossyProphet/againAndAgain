...
function shakePod()
  ...
  activeItem.setArmAngle(armAngle)
    ...
    status.overConsumeResource("health", 0.3)
...
function fire()
  sb.logInfo("%s",status.resource("onMinionCooldown"))
  if self.pets and not self.projectileId and not self.returnProjectileId then
    if arePetsAlive() then
      ...
      setStance("dead")
      if status.resource("onMinionCooldown") ~= 1 then
        jremove(self, "inventoryIcon")
        local memes = {}
        for i,pet in pairs(self.pets) do
        jremove(pet, "status")
        memes[i] = pet
        end
        activeItem.setInstanceValue("pets", memes)
        activeItem.setInstanceValue("currentPets", config.getParameter("pets"))
        activeItem.setInstanceValue("podItemHasPriority", true)
        status.addEphemeralEffect("minioncooldown")
      end
    end
  end
end
