  ...
  for _, chain in pairs(self.chains) do
    ...
    if not continue and (not chain.targetEntityId or world.entityExists(chain.targetEntityId)) then
      ...
      if segmentCount > 0 then
        ...
        local lastPos = {currentBaseOffset[1], currentBaseOffset[2]} --> [[Curve]]
		for i = 1, lastDrawnSegment do
          ...
          if chain.waveform then
            ...
            local sineVal = math.sin(angle)*i/chainLength * chain.waveform.amplitude * 0.5
          ...
          local segmentPos = vec2.add(currentBaseOffset, thisOffset)
		   ...
		   --> Curved start
          if i ~= 1 then
		    local anglePos = vec2.norm(world.distance(lastPos, segmentPos))
		    if chainVec[1] < 0 then
			    anglePos[1] = anglePos[1] * (-1)
		    end
		    segmentAngle = atan2(anglePos[2], anglePos[1])
          end
		   -- Curved end
		  
		  lastPos = {segmentPos[1], segmentPos[2]}
          local drawable = {
            image = image,
            centered = true,
...
--> implementation of atan2 because it does not exist in the math library for some reason
function atan2(y, x)
	if x > 0 then 
