...
function fire(orbIndex)
  ...
  params.powerMultiplier = activeItem.ownerPowerMultiplier()*(1)                 --here be summoner damage boost--
...
function activateShield()
  self.temp = self.shieldEnergyCost
  self.shieldEnergyCost = self.shieldEnergyCost/(status.resource("minionSlotsCount")+1)
  ...
  status.setPersistentEffects("magnorbShield", {{stat = "shieldHealth", amount = self.shieldHealth * status.resource("minionSlotsCount") }})
...
function deactivateShield()
  self.shieldEnergyCost = self.temp
  self.shieldActive = false
