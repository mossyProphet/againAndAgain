...
function WhipCrack:init()
  baseDamage = self.chainDps * self.fireTime
  minionBonus = status.stat("minionStatsBonus") * 0.5
  self.damageConfig.baseDamage = baseDamage * minionBonus
...
function WhipCrack:fire()
  --for i = 1,10 do
    ...
    util.wait(self.stances.fire.duration*4, function()
      if self.damageConfig.baseDamage > 0 then
        self.weapon:setDamage(self.damageConfig, 
        {self.chain.startOffset, 
        {self.chain.startOffset[1] + 0.5, self.chain.startOffset[2]+self.chain.waveform.amplitude},
        {self.chain.endOffset[1], self.chain.endOffset[2]+self.chain.waveform.amplitude*2},  
        {self.chain.endOffset[1] + 0.75, self.chain.endOffset[2]}, 
        {self.chain.endOffset[1], self.chain.endOffset[2]-self.chain.waveform.amplitude*2}, 
        {self.chain.startOffset[1] + 0.5, self.chain.startOffset[2]-self.chain.waveform.amplitude}}, 
        self.fireTime/4)
      end
