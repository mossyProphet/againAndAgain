...
function FlipSlash:init()
  ...
  explodeOnBounce = self.explodeOnBounce
...
function FlipSlash:update(dt, fireMode, shiftHeld)
  ...
  if not self.weapon.currentAbility and self.cooldownTimer == 0 and self.fireMode == "alt" and shiftHeld and not status.statPositive("activeMovementAbilities") and status.overConsumeResource("energy", self.energyUsage) then
    self:setState(self.windup)
  else
    if not self.weapon.currentAbility and self.cooldownTimer == 0 and self.fireMode == "alt" and not shiftHeld and not status.statPositive("activeMovementAbilities") and status.overConsumeResource("energy", self.energyUsage) then
        self:setState(self.windupDef)
    end
  end
end
...
function FlipSlash:flip()
  mcontroller.addMomentum({self.jumpVelocity[1] * self.weapon.aimDirection, self.jumpVelocity[2]})
  ...
  while self.flipTimer < self.flipTime do
    ...
    mcontroller.addMomentum({self.jumpVelocity[1] * self.weapon.aimDirection, self.jumpVelocity[2]})
    if explodeOnBounce then
        local position = mcontroller.position()
        world.spawnProjectile(projectileType, position)
    end
  end
  end
...
function FlipSlash:windupDef()
  ...
  self:setState(self.flipDef)
end

function FlipSlash:flipDef()
  self.weapon:setStance(self.stances.flip)
  self.weapon:updateAim()
