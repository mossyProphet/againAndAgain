...
function getPetPersistentEffects()
  ...
  if onOwnShip and onOwnShip() and recruitSpawner and recruitSpawner.getShipPersistentEffects then
    util.appendLists(effects, recruitSpawner:getShipPersistentEffects())
  end
  return effects  -- Removed unnecessary JSON conversion
end
...
function init()
  ...
  recruitSpawner:init()
  petCount = 0
  petArray = {}  -- Ensure petArray is initialized
  ...
  local ownerUuid = entity.uniqueId()
  if ownerUuid then
    recruitSpawner.ownerUuid = ownerUuid
  else
    sb.logInfo("Warning: entity.uniqueId() returned nil!")
  end
...
function update(dt)
  ...
  if onOwnShip and onOwnShip() then
  ...
  if petSpawner:isDirty() then
    ...
    for uuid in pairs(storage.activePods) do
      local pod = petSpawner.pods[uuid]
      if pod and pod.pets then
        for _, pet in pairs(pod.pets) do
    ...
    playerCompanions.setCompanions("pets", activePets)  -- Removed JSON conversion
