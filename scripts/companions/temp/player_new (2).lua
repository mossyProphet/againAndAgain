...
local petPersistentEffects = {
  ...
  function podsLimit()
	return config.getParameter("activePodLimit") + 1
 end
...
function update(dt)
  ...
  sb.logInfo("%s minions is possible", limit)
  sb.logInfo("%s minions currently active", util.tableSize(storage.activePods))
