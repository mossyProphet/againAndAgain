...
self.lastHealth = status.resourcePercentage("health")
  ...
  self.keepTargetInRange = 200
    self.approachSpeed = config.getParameter("speed", 1) / 5
  self.gravity = -1
  self.targets = {}
  self.outOfSight = {}
  self.notifications = {}
...
repeat
  ...
  if world.gravity(mcontroller.position()) == 0 then
    ...
    self.inGround = true--world.pointCollision({math.floor(mcontroller.position()[1]+0.5), math.floor(mcontroller.position()[2]+0.5)}, {"Block", "Platform", "Dynamic", "Slippery"})
...
function move()
  ...
  animator.rotateTransformationGroup("body", mcontroller.rotation())
   ...
   self.lastPosition = mcontroller.position()
    ...
    --mcontroller.setVelocity(vec2.add(vec2.mul(mcontroller.velocity(), 0.99), vec2.mul(toTarget, 1.1)))
